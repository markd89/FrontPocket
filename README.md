# FrontPocket
FrontPocket provides a front-end to Kyutai Labs Pocket TTS, including the ability to read text from the clipboard, from a file and passed on the CLI. Features include the ability to pause, resume, move back and forward in the spoken text and change playback speeds.

A low-latency, daemon-based text-to-speech system for Linux. FrontPocket loads the TTS model once at startup and streams audio sentence by sentence, so there is minimal delay between sending text and hearing it spoken.
...
Much thanks to the very smart people at Kyutai Labs for their beautiful model and helpful reference code. https://github.com/kyutai-labs/pocket-tts
